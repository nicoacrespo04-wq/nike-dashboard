...
import { factorColor, factorHelp, factorLabel, sortFactors } from "@/lib/viz";
...
/**
 * Barra apilada al 100%: cómo se reparte el score entre los factores DISPONIBLES.
 * Separación de 2px entre segmentos y etiqueta directa en los tramos anchos.
 */
export function ContributionStack({
  ...
  // Las etiquetas directas sólo entran si la barra es lo bastante alta para leerlas.
    ...
    <ul className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
      {ordered.map((f) => (
        <li key={f.factor} className="flex items-center gap-1.5 text-2xs">
          ...
          />
          <span className={f.available ? "text-ink-secondary" : "text-ink-muted line-through"}>
...
export function FactorTable({
  ...
  return (
    <div>
      <div className="grid grid-cols-[minmax(140px,1.4fr)_minmax(90px,0.9fr)_minmax(150px,1.6fr)] items-center gap-3 border-b border-line pb-2 text-2xs font-semibold uppercase tracking-wide text-ink-muted">
        <span>Factor</span>
      ...
      <ul className="divide-y divide-line">
        {ordered.map((f) => {
          ...
          return (
            <li key={f.factor} className={f.available ? "" : "bg-surface-sunken"}>
              <button
                type="button"
                onClick={() => setOpen(isOpen ? null : f.factor)}
                aria-expanded={isOpen}
                className="grid w-full grid-cols-[minmax(140px,1.4fr)_minmax(90px,0.9fr)_minmax(150px,1.6fr)] items-center gap-3 py-2.5 text-left transition hover:bg-surface-sunken"
              >
                {/* Identidad del factor */}
                <span className="flex min-w-0 items-center gap-2">
                  <span
                    aria-hidden
                    className={`inline-block h-3 w-3 shrink-0 rounded-[2px] ${f.available ? "" : "hatch-muted"}`}
                    style={{ backgroundColor: f.available ? factorColor(f.factor) : undefined }}
                  />
                  <span className="min-w-0">
                    <span
                      className={`block truncate text-xs font-semibold ${
                        f.available ? "text-ink-primary" : "text-ink-muted"
                      }`}
                    >
                      {factorLabel(f.factor)}
                    </span>
                ...
                <span className="text-right">
                  {f.available ? (
                    <span className="tabular text-xs font-semibold text-ink-primary">
                      {pctFromFraction(f.raw_score, 0)}
                    </span>
                  ) : (
                    <span className="text-2xs font-medium text-ink-muted">sin datos</span>
                  )}
                </span>
              ...
              {!f.available ? (
                <p className="pb-2.5 pl-5 text-2xs italic leading-relaxed text-ink-muted">
              ...
              {isOpen ? (
                <div className="border-l-2 border-line pb-3 pl-4 text-2xs leading-relaxed text-ink-secondary">
                    ...
                    <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                      {detailEntries.map(([key, value]) => (
                        <div key={key} className="contents">
                          ...
                          <dd className="tabular text-ink-primary">{renderDetailValue(value)}</dd>
                        </div>
                      ))}
                    </dl>
                  ) : (
                    <p className="text-ink-muted">Sin sub-señales registradas para este factor.</p>
                  )}
                </div>
              ) : null}
            </li>
