...
import type { ProductAttribute, ProductDetail } from '@/types/intelligence'
import { getProduct, getProductMatches } from '@/lib/intelligence/api'
...
export default function ProductDetailPage() {
  ...
  const state = useApi((signal) => getProduct(productId, signal), [productId], { enabled })
                  ...
                  <ul className="divide-y divide-surface-border">
                    {data.matches.map((m, i) => (
                      <li key={m.id} className="py-2.5 first:pt-0">
                        <Link href={`/intelligence/matches/${m.id}`} className="block">
